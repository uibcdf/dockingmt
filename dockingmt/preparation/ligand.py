...
class PreparedLigand:
    ...
    def to_pdbqt(self) -> str:
        ...
        lines = ['ROOT']
        for i, (name, (x, y, z), atype, q) in enumerate(
            zip(self.atom_names, coords_ang, self.atom_types, self.charges)
        ):
            line = (
                f'ATOM  {i + 1:5d}  {name:<3s} {self.group_name:3s} A   1    '
...
def prepare_ligand(
    ...
    for i, aname in enumerate(atom_names):
        ...
        if aname_str.startswith('O'):
            ...
            if primary_group in ('BNZ', 'BENZENE') or 'aromatic' in aname_str.lower():
                atype = 'A'
            else:
                atype = 'C'
